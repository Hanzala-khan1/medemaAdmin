  ...
  const queryClient=useQueryClient()
    ...
    const changeImage = async () => {
      ...
      try {
        ...
        console.log(res, "res");
        handleUploadImage();
        await updateDoc(doc(db, "insurance", res.id), {
    ...
    const addData = async (imageUrl) => {
      // setBtnAdd("disable");
      const url = imageUrl;
      const newCityRef = collection(db, "insurance");
    ...
    const updateData = async (docs) => {
      ...
      try {
        ...
        await updateDoc(doc(db, "insurance", formData.id), { ...formData });
    ...
    const deleteData = async (e) => {
      ...
      try {
        ...
        const res = await deleteDoc(doc(db, "insurance", e.id));
        console.log(res);
        setAlertType("error");
    ...
    const { isLoading, data, error } = useQuery(["insurance"], fetchData, {
      staleTime: 60000,
    });
...

      {
        title: (
          <div className="flex items-center w-[260px] justify-center space-x-4">
            ...
            />
            <span className="text-base font-poppins font-medium"> Insurance Name</span>
          </div>
        ),
        dataIndex: "name",
        sorter: (a, b) => a.age - b.age,
    ...
    return (
      <div className="flex flex-col bg-white space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="font-semibold font-barlow text-2xl ml-6">Insurance</h1>
          <button
