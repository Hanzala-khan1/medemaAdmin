...
//   console.log(router.query);
  ...
  const fetchData =async()=>{
    const docRef= query(collection(db,"doctors"),where("auth_id","==", localStorage.getItem("id") ))
try{
    var arr={};
...
console.log(error)
 ...
 const {isLoading,data}=useQuery(["Doctor"],fetchData,{
  staleTime:60000
 })
 if(!isLoading){
  // console.log(data)
    ...
    <div className="" style={{ marginTop: "30px", }}>
     ...
     <div className="mt-[200px] ">
      {!isLoading && data?
          ...
          <h3 className="bg-blue-300 py-2 px-2 text-white" >Fees : {data?.fees?data.fees:"NA"} </h3>
          ...
          <h3 className="bg-blue-400 py-2 px-2 text-white">Discount : {data?.workingAt?data.workingAt:"NA"} </h3>
