import {
  ...
  "AB negative"
   ...
   const addMutaion = useMutation(createUser,{
    ...
    onSuccess:()=>{
      console.log("succes");
      queryClient.invalidateQueries(["Rehab-Doctors"])
    }
   })
    ...
    const handleUploadImage = async (uid) => {
      setLoading(true);
      const imgRef = ref(storage, `doctors/${imgUrl ? imgUrl.uid : ""}`);
    ...
    const fetchData = async () => {
      let arr = [];
      const dbRef = query(collection(db, "doctors"),where("ref","==",localStorage.getItem("id")));
      try {
    ...
    const { isLoading, data, error } = useQuery(["Rehab-Doctors"], fetchData, {
      staleTime: 60000,
    });
        ...
        render: (_, record) => {
          return record?.gender ? (
            <div className="flex items-center justify-center  space-x-2">
              {record.gender == "male" ? (
                <div className="w-[60px] py-1 bg-[#DCEDE5] text-center text-[#3CB43C]">
                  Male
                </div>
              ) : (
                <div className="w-[60px] py-1 bg-[#E7E3F6] text-center text-[#8472CA]">
                  Female
                </div>
              )}
            </div>
        ...
        render: (_, record) => (
          <div className=" flex items-center w-[160px] justify-center">
            <span className="mx-auto text-sm font-poppins font-normal text-[black]  py-1">
              {record?.perDay?record.perDay:"NA" }
            </span>
          </div>
        ),
      },
    ...
    return (
      <div className="flex flex-col bg-white space-y-6">
        ...
        <label className="  ">Name</label>
              ...
              <div className=" col-span-6 ">
              <label className="">Fees/Day</label>
                <Input
                  autoComplete="off"
                  required
                  type="number"
                ...
                />
                 ...
                 <label className="">Fees/Hour</label>
                <Input
                  autoComplete="off"
                  required
                  type="number"
