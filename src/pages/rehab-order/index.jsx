...
setRole(localStorage.getItem("role"))
  ...
  const { isLoading, data, error, } = useQuery(['My-Orders'],  fetchData, {
  ...
  const fetchChildData = async () => {
    ...
    if(localStorage.getItem("role")=="admin"){
      ...
      try {
        ...
        const querySnapshot = await  getDocs(query(dbRef, where("ref", "==", localStorage.getItem("id"))));
  ...
  const childData = useQuery(['Child-Orders'],  fetchChildData, {
   refetchOnWindowFocus:"always"
  });
  ...
  return (
    <div className="flex flex-col bg-white space-y-6">
      ...
      <div className="">
        <Table
          columns={columns}
          dataSource={!isLoading && !childData.isLoading?[...data,...childData.data]:[]}
          onChange={onChange}
          id="newOrders"
          scroll={{ x: 900 }}
        />
