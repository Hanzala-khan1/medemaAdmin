import {
  ...
  const Index = () => {
   ...
   const addMutaion = useMutation(createUser,{
    ...
    onSuccess:()=>{
      console.log("succes");
      queryClient.invalidateQueries(["Rehab-Physio"])
    }
   })
    ...
    const fetchData = async () => {
      let arr = [];
      const dbRef = query(collection(db, "physio"),where("ref","==",localStorage.getItem("id")));
      try {
    ...
    const { isLoading, data, error } = useQuery(["Rehab-Physio"], fetchData, {
      staleTime: 60000,
    });
    ...
    const items = [
      ...
      const columns = [
        ...
        },
        {
          ...
          sorter: (a, b) => a.age - b.age,
          render: (_, record) => (
            <div className=" flex items-center flex-col w-[160px] justify-center">
              <span className="mx-auto text-sm font-poppins font-normal text-[black]  py-1">
                PerDay : {record?.perDay? record.perDay :""}
               
              </span>
              <span className="mx-auto text-sm font-poppins font-normal text-[black]  py-1">
                PerHour : {record?.perHour? record.perHour :""}
