import {
  ...
  "AB negative"
   ...
   const addMutaion = useMutation(createUser,{
    ...
    onSuccess:()=>{
      console.log("succes");
      queryClient.invalidateQueries(["Rehab-Patient"])
    }
   })
   ...
   const insuranceData = useQuery(['Insurance'],getInsurance,{
    ...
    const fetchData = async () => {
      let arr = [];
      const dbRef = query(collection(db, "patient"),where("ref","==",localStorage.getItem("id")));
      try {
    ...
    const { isLoading, data, error } = useQuery(["Rehab-Patient"], fetchData, {
      staleTime: 60000,
    });
