   ...
   const addMutaion = useMutation(createUser,{
    ...
    const updateDocData= async(docs)=>{
      ...
      try{
       ...
       handleCancel()
       queryClient.invalidateQueries(["Physio-profile"])
       MySwal.fire({
        icon: 'success',
        // title: 'Oops...',
...
//   console.log(router.query);
  ...
  const fetchData =async()=>{
    const docRef= query(collection(db,"physio"),where("auth_id","==", localStorage.getItem("id") ))
try{
    var arr={};
...
console.log(error)
 ...
 const {isLoading,data}=useQuery(["Physio-profile"],fetchData,{
  staleTime:60000
 })
 if(!isLoading){
  // console.log(data)
