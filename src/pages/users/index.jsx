...
const Index = () => {
  ...
  const delteImage = async () => {
    const desertRef = ref(storage, `users/${imgName}`);
    ...
    try {
      ...
      await updateDoc(doc(db, "users", formData.id), { images: {} });
    } catch (err) {
      console.log(err);
    }
  };
...
console.log(formData)
    try {
      ...
      await updateDoc(doc(db, "users", formData.id), { ...formData });
     updateDocData(docs)
      MySwal.fire('Updated Data Successfully')
      setInUpdate(false);
      setVisibleAlert(true);
  ...
  const deleteDocData= async(e)=>{
    try{
      const res = await deleteDoc(doc(db, "users", e.id));
      queryClient.invalidateQueries();
      console.log(res)
      MySwal.fire({
        icon: 'success',
  ...
  const deleteData = async (e) => {
    console.log(e);
    try {
      const desertRef = ref(storage, `users/${e.images.name}`);
      const deleteImg = await deleteObject(desertRef);
  ...
  const handleUploadImage = async (id) => {
    ...
    const imgRef = ref(storage, `users/${imgUrl?.uid}`);
  ...
  const fetchData = async () => {
    let arr = [];
    const dbRef = query(collection(db, "users"),where("role","==","user"));
    try {
      ...
      render: (_, record) => (
        <div className="w-full flex justify-center items-center">
          {/* <Avatar
                ...
                /> */}
          <Image
            src={record.photoUrl?record.photoUrl:record?.images?.url?record?.images?.url:loader}
            alt={record.name?record.name:""}
            width={40}
            height={40}
        ...
        return (
          <div className=" flex items-center w-[160px] justify-center">
            <span className="text-sm  text-clip font-poppins font-medium text-[#474747]">
              {/* {record.description} */}
              {record.role ? short.slice(0, 50) : "NA"}
            </span>
          </div>
        );
      },
        ...
        <div className="w-full flex flex-col items-center  justify-center">
          {
            <>
              <span className="mx-auto text-center text-sm font-poppins font-normal text-[black] w-[160px]  py-1">
                Email: {record.email?record.email:"NA"}
              </span>
              <span className="mx-auto text-center  text-sm font-poppins font-normal text-[black] w-[160px]  py-1">
                Phone: {record.phone?record.phone:"NA"}
              </span>
            </>
          }
        </div>
  ...
  return (
    <div className="flex flex-col bg-white space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="font-semibold font-barlow text-2xl ml-6">users</h1>
        <button
