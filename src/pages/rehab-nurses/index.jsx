...
const Index = () => {
  ...
  const addMutaion = useMutation(createUser, {
    onSuccess: () => {
      console.log("succes");
      queryClient.invalidateQueries(["Rehab-Nurses"]);
    },
  });
  ...
  const fetchData = async () => {
    ...
    const dbRef = query(
      ...
      where("ref", "==", localStorage.getItem("id"))
    );
    try {
  ...
  const { isLoading, data, error } = useQuery(["Rehab-Nurses"], fetchData, {
    staleTime: 60000,
  });
