   ...
   const addMutaion = useMutation(createUser,{
    ...
    const updateDocData= async(docs)=>{
      ...
      try{
       ...
       handleCancel()
       queryClient.invalidateQueries(["Aya-profile"])
       MySwal.fire({
        icon: 'success',
        // title: 'Oops...',
...
//   console.log(router.query);
  ...
  const fetchData =async()=>{
    const docRef= query(collection(db,"aya"),where("auth_id","==", localStorage.getItem("id") ))
try{
    var arr={};
...
console.log(error)
 ...
 const {isLoading,data}=useQuery(["Aya-profile"],fetchData,{
  staleTime:60000
 })
 if(!isLoading){
  // console.log(data)
